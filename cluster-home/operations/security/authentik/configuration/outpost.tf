resource "authentik_outpost" "proxy_outpost" {
  ...
  protocol_providers = [
    ...
    resource.authentik_provider_proxy.monitoring_alertmanager.id,
    resource.authentik_provider_proxy.monitoring_grafana.id,
    resource.authentik_provider_proxy.monitoring_prometheus.id,
